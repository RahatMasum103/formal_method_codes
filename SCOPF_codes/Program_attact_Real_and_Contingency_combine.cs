...
namespace SCOPF
{
    class CaOpf
    {
        ...
        const int NLINES = 20;
        const int STEPS = 100;
        const int THRESHOLD_DIFF = 2;   // Generation power can be little over than the actual demand (In Percent)
                ...
                for (int k = 1; k <= nLines; k++)
                {
                    for (int l = 1; l <= nLines; l++)
                    {
                        if (l != k)
                        {
                            ...
                            RealExpr Changed_FLP = (RealExpr)z3.MkSub(FLP[l], Delta_FLP);
