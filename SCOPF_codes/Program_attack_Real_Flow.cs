...
namespace SCOPF
{
    class CaOpf
    {
        ...
        const int NLINES = 7;
        const int STEPS = 50;
        const int THRESHOLD_DIFF = 50;   // Generation power can be little over than the actual demand (In Percent)
        ...
        int nBuses, nLines, nMeasurements, nDBuses, nGBuses, nOverloads, nPercContgn, nPercOverload;  // nBuses also represents the number of states               
       ...
       // IntExpr[] OverLineCountContgn;
        ...
        void Declaration(Context z3)
        {
           ...
           // OverLineCountContgn = new IntExpr[nLines + 1];
            ...
            TotalOverLdCon = (IntExpr)z3.MkConst("TotalOverLoadContingency", z3.IntSort);
        ...
        void ReadInput(Context z3, Optimize o)
        {
            ...
            try
            {
                ...
                #region Number of buses, lines, the reference bus 
                while (true)
                {
                ...
                while (true)
                {
                    ...
                    nOverloads = Convert.ToInt32(lineElement[0]);
                    Console.WriteLine("Minimum Overloaded Lines: {0}%", nOverloads);
                    tw.WriteLine("Minimum Overloaded Lines: {0}%", nOverloads);
                    break;
                }
                ...
                while (true)
                {
                    ...
                    nPercContgn = Convert.ToInt32(lineElement[0]);
                    nPercOverload = Convert.ToInt32(lineElement[1]);

                    Console.WriteLine("% of Contingency: {0} | % of Overload: {1}", nPercContgn, nPercOverload);
                    tw.WriteLine("% of Contingency: {0} | % of Overload: {1}", nPercContgn, nPercOverload);
                    break;
                }
        ...
        void Formalize(Context z3, Optimize opt)
        {
            ...
            #region Get some FLOWS OVER the LINE CAPACITY [CASE - I] 
            {
               ...
               // BoolExpr[] Exprs = new BoolExpr[2 * nLines];
                ...
                for (int l = 1; l <= nLines; l++)
                {
                    ...
                    double MIN_OVERLOADING = 1 + (minLineOverload / 100.0);
                ...
                IntExpr TotalOverLoad = (IntExpr)z3.MkAdd(OverLineCount);
               // RealExpr Percent_Overload = (RealExpr) z3.MkMul(z3.MkInt2Real(TotalOverLoad), z3.MkReal(Convert.ToString(LINE_OVERLOAD)));

                opt.Assert(z3.MkEq(Percent_Overload, z3.MkMul(z3.MkInt2Real(TotalOverLoad), z3.MkReal(Convert.ToString(LINE_OVERLOAD)))));
            ...
            #region FLOWS are in LIMIT of LINE CAPACITY [CASE - II] (COMMENTED now in this file)
            {
                /*
                BoolExpr[] Exprs = new BoolExpr[nLines];
            ...
            for (int k = 1; k <= nLines; k++)
            {
                for (int l = 1; l <= nLines; l++)
                {
                    if (l != k)
                    {
                        ...
                        RealExpr Changed_FLP = (RealExpr)z3.MkAdd(SF_Flow[l], Delta_FLP);
            ...
            */
            ...
            #region Original Flow after Contingency CAPACITY LIMIT (randomly overload line, COMMENTED now) 
            // If after contingency each line is in LIMIT or NOT??
            {
                /*
               // BoolExpr[] Exprs = new BoolExpr[2 * nLines];
            ...
            #region Original Flow after Contingency CAPACITY LIMIT (total overload line on all combination)  
            /*
            {
                BoolExpr[] Exprs = new BoolExpr[nLines];
                for (int i = 1; i <= nLines; i++)
                {
                    ...
                    {
                        double MIN_OVERLOADING = 1 + (minLineOverload / 100.0);
                        OneLineOut[j - 1] = z3.MkOr(z3.MkLt(SF_Lines_Contingency_Flow[i, j], z3.MkReal(Convert.ToString((-1) * maxPL[j] * MIN_OVERLOADING))),
                            z3.MkGt(SF_Lines_Contingency_Flow[i, j], z3.MkReal(Convert.ToString(maxPL[j] * MIN_OVERLOADING))));
                ...
                // Total NUMBER of Overflowed Lines  (implemented, need debug)
              //  opt.Assert(z3.MkEq(OverLineCountContgn[0], z3.MkInt(0)));
              
                IntExpr[] TotalOverLineCountContgn = new IntExpr[nLines];

                for (int i = 1; i <= nLines; i++)
                {
                    IntExpr[] OverLineCountContgn = new IntExpr[nLines];                    
                    //opt.Assert(z3.MkEq(OverLineCountContgn[0], z3.MkInt(0)));
                    for (int j = 1; j <= nLines; j++)
                    {
                        OverLineCountContgn[j-1] = (IntExpr)z3.MkConst("OverFlowContingency_" + (j-1), z3.IntSort);

                        opt.Assert(z3.MkImplies(IsOverFlowContgn[i,j], z3.MkEq(OverLineCountContgn[j-1], z3.MkInt(1))));
                      //  opt.Assert(z3.MkImplies(z3.MkNot(IsOverFlowContgn[i,j]), z3.MkEq(OverLineCountContgn[j-1], z3.MkInt(0))));
                    }
                    TotalOverLineCountContgn[i - 1] = (IntExpr) z3.MkAdd(OverLineCountContgn);
                }
                
                BoolExpr SumOverFlowLineContgn = z3.MkGe(z3.MkAdd(TotalOverLineCountContgn), z3.MkInt(Convert.ToString(TOTAL_OVERLOAD_CONTINGENCY)));
               // opt.Assert(SumOverFlowLineContgn);

                opt.Assert(z3.MkEq(TotalOverLdCon, z3.MkAdd(TotalOverLineCountContgn)));                
        ...
        void Enumerate(Context z3, Optimize opt)
        {
            ...
            while (opt.Check() == Status.SATISFIABLE)           
            {
                ...
                */
                ...
                Console.WriteLine("TOTAL LINES OVERLOAD in CONTINGENCY: " + model.Eval(TotalOverLdCon).ToString());
                tw.WriteLine("TOTAL LINES OVERLOAD in CONTINGENCY:" + model.Eval(TotalOverLdCon).ToString());
                Console.WriteLine();
                */
                /*
