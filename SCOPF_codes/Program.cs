...
namespace SCOPF
{
    class CaOpf
    {
        ...
        const double TOTAL_OVERLOAD_CONTINGENCY = 1;
        ...
        void Formalize(Context z3, Optimize opt)
        {
            ...
            #region Get some flows over the line capcaity [DONE: Input the minimum level of overloading]
            {
               ...
               // BoolExpr[] Exprs = new BoolExpr[2 * nLines];
                ...
                BoolExpr SumOverFlowLine = z3.MkGe(z3.MkAdd(OverLineCount), z3.MkInt(Convert.ToString(nOverloads)));
            ...
            #region Original FLOW int CONTINGENCY

            #region Original FLOW (n-1) Contingency (Commented for now)
            for (int k = 1; k <= nLines; k++)
            {
                for (int l = 1; l <= nLines; l++)
                {
            ...
            */
            ...
            #region Original Flow after Contingency CAPACITY LIMIT (randomly overload line, CASE-I)
            // If after contingency each line is in LIMIT or NOT??
            {
               // BoolExpr[] Exprs = new BoolExpr[2 * nLines];
                BoolExpr[] Exprs = new BoolExpr[nLines];
            ...
            #region Original Flow after Contingency CAPACITY LIMIT (total overload line, CASE-II)            
            {
                for (int i = 1; i <= nLines; i++)
                {
                    BoolExpr[] OneLineOut = new BoolExpr[nLines];
                ...
                for (int i = 1; i <= nLines; i++)
                {
                    ...
                    for (int j = 1; j <= nLines; j++)
                    {
                       // opt.Assert(z3.MkImplies(IsOverFlowContgn[i,j], z3.MkEq(OverLineCountContgn[j], z3.MkInt(1))));
                       // opt.Assert(z3.MkImplies(z3.MkNot(IsOverFlowContgn[i,j]), z3.MkEq(OverLineCountContgn[j], z3.MkInt(0))));
                    }
                    TotalOverLineCountContgn[i - 1] = (IntExpr) z3.MkAdd(OverLineCountContgn);
                }
                BoolExpr SumOverFlowLineCon = z3.MkGe(z3.MkAdd(TotalOverLineCountContgn), z3.MkInt(Convert.ToString(TOTAL_OVERLOAD_CONTINGENCY)));
                opt.Assert(SumOverFlowLineCon);
